    ...
    def aggregate(
        ...
        return AggregatedVerification(
            ...
            processing_metadata={
                ...
                "extraction_source": extraction_metadata.get("extraction_source", "FRESH" if str(extraction_metadata.get("mode", "")).upper() == "LIVE" else ("MOCK" if str(extraction_metadata.get("mode", "")).upper() == "MOCK" else "CACHED")),
                "cache_policy": extraction_metadata.get("cache_policy", "BYPASS_CACHE" if str(extraction_metadata.get("mode", "")).upper() == "LIVE" else ("DISABLED" if str(extraction_metadata.get("mode", "")).upper() == "MOCK" else "USE_CACHE")),
                **{k: v for k, v in extraction_metadata.items() if k not in ["mode", "model"]},
            },
